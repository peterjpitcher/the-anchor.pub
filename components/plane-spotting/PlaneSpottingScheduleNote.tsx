...
export function PlaneSpottingScheduleNote({
  ...
  if (variant === 'panel') {
    return (
      <div className={cn(
        'rounded-lg border border-anchor-gold/20 bg-anchor-bg-card p-4 text-anchor-cream-text shadow-sm',
        className
      )}>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex gap-3">
            <span className="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-anchor-gold/30 bg-anchor-gold/10 text-anchor-gold-vivid">
              <Plane className="h-5 w-5" aria-hidden="true" />
            </span>
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-anchor-gold-vivid">
              ...
              <p className="mt-2 flex gap-2 text-sm text-anchor-cream-text/70">
                <CloudSun className="mt-0.5 h-4 w-4 shrink-0 text-anchor-gold-vivid" aria-hidden="true" />
          ...
          {showCta ? (
            <Link
              ...
              className="inline-flex min-h-[44px] w-full items-center justify-center rounded-full bg-anchor-gold px-6 py-3 text-base font-semibold text-white transition-colors hover:bg-anchor-gold-light focus:outline-none focus:ring-2 focus:ring-anchor-gold focus:ring-offset-2 md:w-auto"
            >
              Book a Table
            </Link>
          ) : null}
  ...
  return (
    <div className={cn(
      'rounded-lg border border-anchor-gold/15 bg-anchor-bg-raised/50 p-3 text-sm text-anchor-cream-text/80',
      className
    )}>
