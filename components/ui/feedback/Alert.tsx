...
const alertVariants = cva(
  'relative w-full rounded-md p-4 flex items-start gap-3',
  {
    variants: {
      variant: {
        info: 'bg-surface text-ink border border-line',
        success: 'bg-anchor-success/10 text-ink border border-anchor-success/30',
        warning: 'bg-anchor-gold/10 text-ink border border-anchor-gold/40',
        error: 'bg-anchor-danger/10 text-ink border border-anchor-danger/30'
      }
    },
    defaultVariants: {
      variant: 'info'
...
const iconMap: Record<Status, React.ReactNode> = {
  ...
  success: (
    <svg className="w-5 h-5 flex-shrink-0 text-anchor-success" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
    </svg>
  ),
  warning: (
    <svg className="w-5 h-5 flex-shrink-0 text-accent-text" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
    </svg>
  ),
  error: (
    <svg className="w-5 h-5 flex-shrink-0 text-anchor-danger" fill="currentColor" viewBox="0 0 20 20">
