...
- **These closures are not yet in the management API.** Checked 8 September 2026: `GET /business/hours` returns `planning.nextClosure: null` and a single `specialHours` row (31 October, Halloween). Until `special_hours` rows exist for **2026-12-25, 2026-12-26 and 2027-01-01**, the site resolves all three to regular hours and will tell people we are open, with a kitchen, when we are shut. The SSOT is the record of intent; the API is what the site actually serves.
