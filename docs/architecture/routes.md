...
| `/api/table-bookings/[reference]` | GET, DELETE | None (reference-based) | Look up or cancel booking by reference |
...
| Path | Method | Auth | Purpose |
|------|--------|------|---------|
| `/api/business/hours` | GET | ANCHOR_API_KEY (no-cache) | Current opening hours + special hours |
...
| `/api/parking/bookings` | POST | None | Create parking booking |
| `/api/parking/bookings/[id]` | GET | None (public read) | Get parking booking |
| `/api/parking/payment/create-order` | POST | None | Create PayPal order for parking |
| `/api/parking/payment/capture` | POST | None | Capture parking payment |
...
| `/api/enquiry/christmas` | POST | ANCHOR_API_KEY | Send Christmas enquiry email |
...
- Cache headers: `/api/business/hours` is `no-store`; other API routes get `s-maxage=60, stale-while-revalidate=300`; static assets get `max-age=31536000, immutable`
