...
- [ ] Add the Christmas Day fact to `docs/SSOT.md` section 7 and to `SSOT.json`: open for drinks 12pm to 3pm, no food.
- [ ] Add a Christmas Day FAQ to `/christmas-parties`, since the offer window ends 20 December and the page is currently silent.
- [ ] Extend `tests/ssot-drift-guard.test.ts` to pin the fact and ban a food claim on Christmas Day.
...
- [ ] `lib/christmas-season.ts`: replace the single `CHRISTMAS_MINIMUM_PARTY_SIZE` with a midweek value (4, Tuesday to Thursday) and a weekend value (6, Friday and Saturday), plus `getChristmasMinimumPartySize(isoDate)`.
- [ ] Update `docs/SSOT.md` and `SSOT.json`.
- [ ] `app/api/enquiry/christmas/route.ts:531-539`: server validation must use the minimum for the requested date, not a flat 6.
- [ ] `app/christmas-parties/client-components.tsx`: the form's `min` and its error copy follow the selected date.
- [ ] Fix every page that states a flat "6 guests or more": `/christmas-parties`, `app/page.tsx:287`, `app/corporate-events/page.tsx:245,514`, `app/food-menu/page.tsx:359`, `app/beer-garden/page.tsx:401`.
- [ ] Tests for the boundary: 4 accepted on a Wednesday, rejected on a Saturday.
...
- [ ] Drinks-only Christmas party: state that there is no minimum spend and that it is arranged as a private booking so the pub can confirm nothing else is needed.
- [ ] 21 to 29 seated guests: explain that it is handled as a private booking because of the no-show exposure at that size, worded as a service rather than a refusal.

## Wave 4: the two noindexed blog posts

- [ ] `content/blog/christmas-events/index.md`: rewrite to 2026 facts, remove `noindex`, add to `app/sitemap.ts`.
- [ ] Verify live after deploy that it returns an indexable robots tag.
