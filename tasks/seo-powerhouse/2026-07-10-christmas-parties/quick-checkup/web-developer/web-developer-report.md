...
The page is **not yet a fully approved release** because the stale price-bearing Event schema and unverified testimonials require owner decisions. GA4/GTM, consent and GSC work is blocked by external access.

Aside from those approval and access items, the code is technically ready. The duplicate selector was removed, selected states now use `aria-pressed`, runtime enum/range validation was added, and a mocked route suite proves management-first delivery with Graph email as fallback. UX also confirmed the mocked mobile success state is visible without sending a real request. The final lint, TypeScript, full test and production-build checks pass from the stable source.

## Verification reviewed

- `npm run lint`: passed with no warnings or errors.
- `npx tsc --noEmit`: passed after the final production build.
- Expanded targeted Christmas/API/SSOT suites: 45 tests passed. The forced management-failure test logs its expected error before proving Graph fallback.
- Full Jest run: 99 suites passed, 930 tests passed and 1 skipped. Existing unrelated React `act(...)` warnings remain.
- Final production build: passed and emitted `/christmas-parties` as a static route at 17.4 kB page size and 182 kB first-load JS. These are build outputs, not CWV results.
...
| Structured data | Easy to change, currently inaccurate | Breadcrumb and FAQ schema use shared components/utilities. The Christmas Event is a page-local object passed to the client, so replacement is technically easy. It remains high-risk because it changes material rich-result/entity markup on a commercial page. |
...
| Replace stale Event/offers schema | Technical, Editor | One-off page fix | High-risk approval required | Technically easy | 1 | 4 | Batched schema approval |
| Remove unverified testimonials unless source and permission records are supplied | Editor, UX | Content process fix | High-risk approval required | Easy | 1 | 3 | Owner sources or removal approval |
...
1. Approval-gated only: replace the page-local Event/offers object with non-priced markup that exactly matches visible approved content.
...
There is no URL, canonical, robots, redirect or indexation migration. The remaining risks are content/schema and operational:
...
2. **One approval batch, Christmas trust:** schema replacement and the testimonial decision. Keep any later page pruning separate and evidence-led.
...
Request one decision covering:

- replacing the long-running price-bearing Event with non-priced page/service markup;
- removing the three unsourced testimonials unless provenance is supplied;
- preparing a later keep/merge/remove reduction of repetitive FAQ/corporate/party-idea content.

Risk if wrong: loss of valid search coverage, useful proof or enhancement eligibility. Rollback: keep the old schema and all removed copy in version control/a dated rollback document; restore only the affected block if verified search or qualified-enquiry performance declines.
...
- Testimonial retention: blocked until original sources and permission are supplied.
- CWV field evidence: unavailable; collect after release without claiming a pass or failure now.

```json
{"findings":[{"finding":"Email fallback protects the lead but can leave the management system without a booking record.","evidence":"app/api/enquiry/christmas/route.ts returns delivery email_fallback after a failed management request and successful Graph mail; tests/api/christmas-enquiry.test.ts verifies this branch.","source":"Current route and Jest inspection","dataStatus":"Known","severity":"Medium","confidence":"High","impactArea":"revenue","owner":"Technical","effort":"Small","dependencies":"Operational monitoring and reconciliation owner","fixType":"Analytics/governance fix","recommendedAction":"Alert on or report email_fallback responses and reconcile them into the management system.","validationStep":"Force a staging management 500, confirm one fallback email and confirm operations can identify and recover the missing record.","riskRollback":"Do not remove the fallback; disable only new alerting if it produces false positives."},{"finding":"Material schema and testimonial decisions remain approval-gated on a commercial page.","evidence":"app/christmas-parties/page.tsx still contains price-bearing Event/offers markup, and editor-qa-report.md identifies three testimonials without source or permission records.","source":"Current code, specialist reports and approval audit","dataStatus":"Known","severity":"Critical","confidence":"High","impactArea":"SEO","owner":"Editorial","effort":"Small","dependencies":"Owner approval or approved source records","fixType":"One-off page fix","recommendedAction":"Approve accurate non-priced page/service schema and either supply testimonial provenance or approve removal.","validationStep":"Re-crawl rendered copy and JSON-LD, then reconcile every retained testimonial to its source record.","riskRollback":"Restore only the verified affected block from version control if a valid issue appears after release."},{"finding":"Search and conversion measurement cannot yet validate the release.","evidence":"The published GTM container did not send the page custom funnel events to GA4 in the analytics specialist test; no GA4 property access, DebugView proof or exact-page GSC export exists in the workspace.","source":"Analytics specialist browser inspection and workspace evidence review","dataStatus":"unavailable","severity":"High","confidence":"High","impactArea":"SEO","owner":"Analytics","effort":"Small","dependencies":"GTM, GA4, GSC and privacy-owner access","fixType":"Analytics/governance fix","recommendedAction":"Map form completion to GA4 generate_lead, map supporting events with consent controls, verify the canonical GSC property and capture a dated baseline.","validationStep":"Use Tag Assistant, network and GA4 DebugView for party, lunch and dinner, then reconcile a GSC exact-page/query export with the UI.","riskRollback":"Duplicate mappings can inflate leads; publish a versioned GTM container and revert it if any controlled action fires more than once."}]}
```
